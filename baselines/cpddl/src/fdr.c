...
{
    int *ordering = ZALLOC_ARR(int, fdr->var.var_size + 1);
    pddl_cg_t cg;
    pddlCGInit(&cg, &fdr->var, &fdr->op, 0);
    pddlCGVarOrdering(&cg, &fdr->goal, ordering);
    pddlCGFree(&cg);

    int *remap = ZALLOC_ARR(int, fdr->var.var_size);
    for (int vi = 0; vi < fdr->var.var_size; ++vi){
...
int pddlFDRIsRelaxedPlan(const pddl_fdr_t *fdr,
                         ...
                         pddl_err_t *err)
{
    int *reached = ZALLOC_ARR(int, fdr->var.global_id_size);
                  ...
                  int op_id)
{
    ...
    fdr_op->is_aux_remove_from_plan = op->is_aux_remove_from_plan;
...
static void tnfFull(pddl_fdr_t *fdr,
                    ...
                    pddl_err_t *err)
{
    pddl_fdr_val_t **u_vals = ZALLOC_ARR(pddl_fdr_val_t *,
                                         fdr->var.var_size);
