                   ...
                   pddl_err_t *err)
{
    ...
    if (pddlDisambiguate(&constr->disambiguate, &op->pre, NULL,
                         1, 0, NULL, &op->pre) < 0){
        LOG(err, "Operator %d:(%s) skipped, because it"
                      ...
                      pddl_err_t *err)
{
    ...
    pddl_bdd_t **bdds = ZALLOC_ARR(pddl_bdd_t *, 2 * vars->group_size);
    int *pre_set = ZALLOC_ARR(int, vars->group_size);
    int *eff_set = ZALLOC_ARR(int, vars->group_size);
                              ...
                              pddl_err_t *err)
{
    ...
    pddl_symbolic_trans_t *T = ZALLOC_ARR(pddl_symbolic_trans_t, T_size);
    int Tres_size = 0;
    pddl_symbolic_trans_t *Tres = ZALLOC_ARR(pddl_symbolic_trans_t, T_size);
    for (int i = 0; i < T_size; ++i)
        transInit(vars, constr, ops + op_ids[i], T + i, use_op_constr, err);

    LOG(err, "Initialized individual trans BDDs: cost: %s,"
              " heur change: %s ops: %d",
              F_COST(&trset->cost),
    ...
    trset->trans = ZALLOC_ARR(pddl_symbolic_trans_t, trset->trans_size);
                               ...
                               pddl_err_t *err)
{
    ...
    op_t *ops = ZALLOC_ARR(op_t, strips->op.op_size);
    opsInit(constr, strips, use_op_constr, ops,
            op_heur_change, sum_op_heur_change_to_cost, err);

    trset->trans_size = 0;
    trset->trans_alloc = 2;
    trset->trans = ZALLOC_ARR(pddl_symbolic_trans_set_t, trset->trans_alloc);
