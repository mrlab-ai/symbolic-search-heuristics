                         ...
                         pddl_err_t *err)
{
    ...
    ASSERT(g->strips_maker.ground_atom.atom_size == 0);
    ASSERT(g->strips_maker.ground_atom_static.atom_size == 0);

    // Insert initial state
    if (pddlStripsMakerAddInit(&g->strips_maker, g->pddl) != 0)
        TRACE_RET(err, -1);

    for (int gai = 0; gai < g->strips_maker.ground_atom.atom_size; ++gai){
        const pddl_ground_atom_t *ga = g->strips_maker.ground_atom.atom[gai];
        pddlSqlGrounderInsertGroundAtom(g->grounder, ga, err);
    }

    for (int gai = 0; gai < g->strips_maker.ground_atom_static.atom_size; ++gai){
        const pddl_ground_atom_t *ga = g->strips_maker.ground_atom_static.atom[gai];
        pddlSqlGrounderInsertGroundAtom(g->grounder, ga, err);
    }
