                ...
                int eff_eff_edges)
{
    ...
    int *value = ZALLOC_ARR(int, vars->var_size * vars->var_size);
    PDDL_ISET(pre);
    PDDL_ISET(eff);
    ...
    cg->node = ZALLOC_ARR(pddl_cg_node_t, cg->node_size);
    for (int v1 = 0; v1 < vars->var_size; ++v1){
        ...
        int ins_fw = 0;
        int ins_bw = 0;
        n1->fw = ZALLOC_ARR(pddl_cg_edge_t, n1->fw_size);
        n1->bw = ZALLOC_ARR(pddl_cg_edge_t, n1->bw_size);
        for (int v2 = 0; v2 < vars->var_size; ++v2){
                             ...
                             const pddl_iset_t *vars_set)
{
    ...
    int *vars = ZALLOC_ARR(int, src->node_size);
    int var;
                                    ...
                                    int *var_ordering)
{
    ...
    int *important = ZALLOC_ARR(int, cg->node_size);
    if (goal == NULL){
        for (int v = 0; v < cg->node_size; ++v)
            important[v] = 1;
    }else{
