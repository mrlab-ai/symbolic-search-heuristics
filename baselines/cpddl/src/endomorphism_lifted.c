...
{
    int *counted = ZALLOC_ARR(int, pddl->obj.obj_size);
    int covered = 0;
...
static int fixConstants(pddl_fm_t *c, void *_end)
{
    ...
    if (pddlFmIsAtom(c)){
        ...
        pddl_fm_atom_t *atom = pddlFmNumExpGetFluentAtom(e);
        fixAtomConstants(atom, end);
    }
...
static void liftedEndomorphismAnalyzeAction(
                ...
                pddl_err_t *err)
{
    pddlFmTraverseAll((pddl_fm_t *)act_pre, NULL, fixConstants, end);
    pddlFmTraverseAll((pddl_fm_t *)act_eff, NULL, fixConstants, end);
                                   ...
                                   pddl_err_t *err)
{
    ...
    end->obj_is_fixed = ZALLOC_ARR(int, end->obj_size);

    // Set objects in the goal as fixed
    pddlFmTraverseProp((pddl_fm_t *)pddl->goal, NULL,
...
static void predObjTuplesInit(pred_obj_tuples_t *tup, const pddl_t *pddl)
{
    ...
    tup->tuple = ZALLOC_ARR(pred_obj_tuple_t, tup->pred_size);
    int i;
...
static int _predObjTuplesInitFromCond(pddl_fm_t *c, void *u)
{
    ...
    }else if (pddlFmIsNumCmpSimpleGroundEq(c)){
        const pddl_fm_atom_t *fluent;
        const pddl_fm_num_exp_t *value;
        pddlFmNumCmpCheckSimpleGroundEq(c, &fluent, &value);
        ASSERT(fluent != NULL);
        ASSERT(pddlFmAtomIsGrounded(fluent));
        if (value->fm.type == PDDL_FM_NUM_EXP_FNUM)
            PANIC("Lifted endomorphisms supports only integer constant values.");
        if (value->e.inum < 0)
            PANIC("Lifted endomorphisms supports only non-negative integer values.");

        int pred_id = fluent->pred + tup->func_offset;
        pred_obj_tuple_t *tuple = tup->tuple + pred_id;
        ASSERT(fluent->arg_size == tuple->size);
        obj_tuple_t *t = predObjTupleAdd(tuple);
        t->value = value->e.inum;
        for (int i = 0; i < fluent->arg_size; ++i){
            ASSERT(fluent->arg[i].obj >= 0);
            t->tuple[i] = fluent->arg[i].obj;
        }
                                      ...
                                      const pddl_fm_t *cond)
{
    ...
    pddlFmTraverseProp((pddl_fm_t *)cond, NULL,
...
static int extractSol(int obj_size,
                      ...
                      int *map)
{
    int *mapped_to = ZALLOC_ARR(int, obj_size);
                              ...
                              const pddl_lifted_mgroups_t *lifted_mgroups)
{
    ...
    select->mgroup_used = ZALLOC_ARR(int, select->mgroup_size);
    select->obj_size = pddl->obj.obj_size;
    select->obj_st = ZALLOC_ARR(int, select->obj_size);
