                          ...
                          const pddl_op_mutex_pairs_t *op_mutex)
{
    ...
    rgen->op_mutex_with = ZALLOC_ARR(pddl_iset_t, rgen->op_size);
                       ...
                       const pddl_op_mutex_pairs_t *op_mutex)
{
    ...
    red->op_mutex_with = ZALLOC_ARR(pddl_iset_t, red->op_size);
    for (int op_id = 0; op_id < red->op_size; ++op_id){
    ...
    red->gen = ZALLOC_ARR(reduce_gen_t, sym->gen_size);
