                          ...
                          const pddl_mgroups_t *mgroups)
{
    ...
    vars->group = ZALLOC_ARR(pddl_symbolic_fact_group_t, vars->group_size);
    vars->fact_size = fact_size;
    vars->fact = ZALLOC_ARR(pddl_symbolic_fact_t, vars->fact_size);
                                   ...
                                   int *var_size)
{
    ...
    *var_pre = ZALLOC_ARR(pddl_bdd_t *, *var_size);
    *var_eff = ZALLOC_ARR(pddl_bdd_t *, *var_size);
    int ins = 0;
    PDDL_ISET_FOR_EACH(groups, group_id){
