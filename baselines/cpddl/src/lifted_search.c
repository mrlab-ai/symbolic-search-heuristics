...
static int applyAction(pddl_lifted_search_t *s,
...
static pddl_lifted_search_status_t bfsStep(pddl_lifted_search_t *s)
{
    ...
    for (int app_i = 0; app_i < app_size; ++app_i){
        ...
        int cost, args_id;
        if (applyAction(s, action, args, &args_id, &cost, s->err) != 0){
            CTXEND(s->err);
            return PDDL_LIFTED_SEARCH_ABORT;
        }
        if (args_id < 0)
            continue;
...
static void setGoal(pddl_lifted_search_t *s)
{
    ...
    pddlFmTraverseProp(s->pddl->goal, _setGoal, NULL, s);
}
...
static pddl_state_id_t insertInitState(pddl_lifted_search_t *s)
{
    const pddl_t *pddl = s->pddl;
    PDDL_ISET(init);
    if (pddlStripsMakerAddInitAndCollect(&s->strips, pddl, &init, NULL) != 0)
        return PDDL_NO_STATE_ID;
                       ...
                       pddl_err_t *err)
{
    *args_id = -1;
    PDDL_ISET(add_eff);
    PDDL_ISET(del_eff);
    int st = pddlStripsMakerActionEffInState(&s->strips, s->pddl, action, args,
                                             &s->cur_node.state, &add_eff,
                                             &del_eff, cost, err);
    if (st != 0){
