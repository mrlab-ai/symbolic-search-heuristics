...
static int actionInitEff(pddl_fm_t *c, void *ud)
{
    ...
    if (pddlFmIsAtom(c)){
        ...
        PDDL_ERR(ctx->err, "Numerical comparators are not supported in operators' effects.");
        ctx->failed = 1;
        return -2;
        ...
        int ival;
        int st = pddlFmNumOpCheckSimpleIncreaseTotalCost(c, ctx->pddl->func.total_cost_func,
                                                         &atom, &ival);
        if (st == 0){
            if (atom != NULL){
                pddlFmArrAdd(&ctx->a->increase_total_cost_atom, &atom->fm);
            }else{
                ctx->a->increase_total_cost_value += ival;
            }
        }else{
            PDDL_ERR(ctx-> err, "Effect contains numerical operator different"
                     " from (increase (total-cost) atom-or-integer-constant)."
        ...
        PDDL_ERR(ctx-> err, "Effect is not a simple conjuction"
                  " (possibly containing conditional effects and function"
                       ...
                       pddl_err_t *err)
{
    ...
    pddlFmTraverseProp(pre, actionInitPre, NULL, &ctx);
    if (ctx.failed){
        PDDL_TRACE_PREPEND_RET(err, -1, "Prepapration of action %s failed: ",
                               action->name);
    }

    pddlFmTraverseProp(eff, actionInitEff, NULL, &ctx);
    if (ctx.failed){
        PDDL_TRACE_PREPEND_RET(err, -1, "Prepapration of action %s failed: ",
                               action->name);
    }
...
static void actionFree(pddl_prep_action_t *a)
{
    ...
    pddlFmArrFree(&a->increase_total_cost_atom);
                             ...
                             const pddl_t *pddl, pddl_err_t *err)
{
    ...
    pddlFmTraverseProp((pddl_fm_t *)ctx.action->eff,
