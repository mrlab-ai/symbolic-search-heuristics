                   ...
                   pddl_err_t *err)
{
    ...
    h3->meta_fact1 = ZALLOC_ARR(char, h3->fact_size);
    h3->meta_fact2 = ZALLOC_ARR(char, h3->fact_size * h3->fact_size);
    ...
    if (meta_fact3_size <= max_mem){
        h3->meta_fact3 = ZALLOC_ARR(char, meta_fact3_size);
        max_mem -= meta_fact3_size;
        used_excess_mem += meta_fact3_size;
    }else{
        h3->meta_fact3_set = ZALLOC_ARR(set_range_t,
    ...
    if (op_fact1_size <= max_mem){
        h3->op_fact1 = ZALLOC_ARR(char, op_fact1_size);
    ...
    if (op_fact2_size <= max_mem){
        h3->op_fact2 = ZALLOC_ARR(char, op_fact2_size);
    ...
    h3->op_applied = ZALLOC_ARR(int, h3->op_size);
