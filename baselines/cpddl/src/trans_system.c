                          ...
                          const pddl_mutex_pairs_t *mutex)
{
    ...
    tss->fact_to_mgroup = ZALLOC_ARR(pddl_mgroup_idx_pairs_t,
                                     tss->fact_size);
                              ...
                              const pddl_trans_system_abstr_map_t *map)
{
    ...
    pddl_iset_t *labels = ZALLOC_ARR(pddl_iset_t, labels_size);
...
int pddlTransSystemsCollectDeadLabels(pddl_trans_systems_t *tss, int ts_id)
{
    ...
    int *label = ZALLOC_ARR(int, tss->label.label_size);
    for (int ltri = 0; ltri < ts->trans.trans_size; ++ltri){
        const pddl_labeled_transitions_t *ltr = ts->trans.trans + ltri;
        int lbl;
                                          ...
                                          const pddl_iset_t *mgroup)
{
    pddl_trans_systems_t *tss = ts->trans_systems;
    int mgroup_size = pddlISetSize(mgroup);
    pddl_iset_t *in_ops = ZALLOC_ARR(pddl_iset_t, mgroup_size);
    pddl_iset_t *out_ops = ZALLOC_ARR(pddl_iset_t, mgroup_size);
    pddl_iset_t *loop = ZALLOC_ARR(pddl_iset_t, mgroup_size);
    PDDL_ISET(ops);
