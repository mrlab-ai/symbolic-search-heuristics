                            ...
                            const pddl_mgroups_t *mgroups)
{
    ...
    pddl_iset_t *from = ZALLOC_ARR(pddl_iset_t, bv->fact_vertex_size);
    pddl_iset_t *to = ZALLOC_ARR(pddl_iset_t, bv->fact_vertex_size);
                ...
                pddl_err_t *err)
{
    ...
    pddl_iset_t *mgs = ZALLOC_ARR(pddl_iset_t, mgroups->mgroup_size);
    pddl_iset_t *mgs_vert = ZALLOC_ARR(pddl_iset_t, mgroups->mgroup_size);
                ...
                pddl_err_t *err)
{
    ...
    int *conflicts = ZALLOC_ARR(int, strips->fact.fact_size);
                          ...
                          pddl_err_t *err)
{
    ...
    // Prepare vertices
    bv->fact_to_fact_vertex = ZALLOC_ARR(pddl_iset_t, bv->fact_size);
    bv->fact_vertex_size = numFactVertices(mgroups, &bv->invertible_facts);
    LOG(err, "Fact-mgroup pairs: %d", bv->fact_vertex_size);
    bv->fact_vertex = ZALLOC_ARR(fact_vertex_t, bv->fact_vertex_size);
    for (int vert_id = 0; vert_id < bv->fact_vertex_size; ++vert_id){
                        ...
                        const pddl_iset_t *black_vars)
{
    ...
    int *black = ZALLOC_ARR(int, bv->fact_vertex_size);
    int var;
                   ...
                   pddl_err_t *err)
{
    ...
    sol.var_val = ZALLOC_ARR(double, bv->fact_vertex_size);
                              ...
                              pddl_err_t *err)
{
    ...
    pddl_iset_t *fact_op = ZALLOC_ARR(pddl_iset_t, strips->fact.fact_size);
    for (int fact = 0; fact < strips->fact.fact_size; ++fact){
    ...
    pddl_iset_t *bmgroups = ZALLOC_ARR(pddl_iset_t, num_mgroups);
    pddl_iset_t *bmgroups_vert = ZALLOC_ARR(pddl_iset_t, num_mgroups);
    int vert_id;
    PDDL_ISET_FOR_EACH(black_vars, vert_id){
