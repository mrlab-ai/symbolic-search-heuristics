                              ...
                              pddl_time_limit_t *time_limit)
{
    ...
    pddl_iset_t *state_allow = ZALLOC_ARR(pddl_iset_t, ts->num_states);
...
static int tsPresolve(ts_presolve_t *presolve,
                      ...
                      pddl_err_t *err)
{
    presolve->op_identity = ZALLOC_ARR(int, tss->label.label_size);
    presolve->op_allow = ZALLOC_ARR(int *, tss->label.label_size);
    presolve->state_allow = ZALLOC_ARR(int **, tss->ts_size);
    for (int tsi = 0; tsi < tss->ts_size; ++tsi){
        if (pddlTimeLimitCheck(time_limit) != 0)
            return -1;
        int num_states = tss->ts[tsi]->num_states;
        presolve->state_allow[tsi] = ZALLOC_ARR(int *, num_states);
        for (int si = 0; si < tss->ts[tsi]->num_states; ++si)
            presolve->state_allow[tsi][si] = ZALLOC_ARR(int, num_states);
    }

    label_groups_t *label_group;
    label_group = ZALLOC_ARR(label_groups_t, tss->ts_size);

    iset_ptr_arr_t *relevant = ZALLOC_ARR(iset_ptr_arr_t, tss->label.label_size);
    for (int tsi = 0; tsi < tss->ts_size; ++tsi){
        if (pddlTimeLimitCheck(time_limit) != 0){
    ...
    for (int op_id = 0; op_id < tss->label.label_size; ++op_id){
        ...
        if (pddlISetSize(&allowed2) <= 1){
            ...
            presolve->op_allow[op_id] = ZALLOC_ARR(int, tss->label.label_size);
                       ...
                       pddl_endomorphism_sol_t *sol)
{
    ...
    int *mapped_to = ZALLOC_ARR(int, label_size);
    for (int oi = 0; oi < label_size; ++oi)
