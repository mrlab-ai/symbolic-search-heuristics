                             ...
                             pddl_err_t *err)
{
    ...
    lt->action = ZALLOC_ARR(pddl_asnets_action_t, lt->action_size);

    lt->pred_size = lt->pddl.pred.pred_size;
    lt->pred = ZALLOC_ARR(pddl_asnets_pred_t, lt->pred_size);
                                     ...
                                     pddl_err_t *err)
{
    ...
    gt->op = ZALLOC_ARR(pddl_asnets_op_t, gt->op_size);
    ...
    gt->fact = ZALLOC_ARR(pddl_asnets_fact_t, gt->fact_size);
    for (int i = 0; i < gt->fact_size; ++i){
        ...
        gt->fact[i].related_op = ZALLOC_ARR(pddl_iarr_t, gt->fact[i].related_op_size);
    }
                             ...
                             pddl_err_t *err)
{
    ...
    int *fact_in_fdr = ZALLOC_ARR(int, gt->strips.fact.fact_size);
