...
{
    int *op_used = ZALLOC_ARR(int, op_size);
                                ...
                                const unsigned int *aut)
{
    ...
    gen->fact = ZALLOC_ARR(int, fact_size);
    gen->fact_inv = ZALLOC_ARR(int, fact_size);
    for (int i = 0; i < fact_size; ++i){
        gen->fact[i] = i;
        gen->fact_inv[i] = i;
    }
    gen->op = ZALLOC_ARR(int, op_size);
    gen->op_inv = ZALLOC_ARR(int, op_size);
    for (int i = 0; i < op_size; ++i){
        gen->op[i] = i;
        gen->op_inv[i] = i;
    }
...
void pddlStripsSymPrintDebug(const pddl_strips_sym_t *sym, FILE *fout)
{
    ...
    fact_used = ZALLOC_ARR(int, sym->fact_size);
