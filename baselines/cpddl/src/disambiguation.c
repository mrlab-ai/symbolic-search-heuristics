                         ...
                         const pddl_mgroups_t *mgroup_in)
{
    ...
    dis->fact = ZALLOC_ARR(pddl_disambiguate_fact_t, dis->fact_size);
    ...
    dis->mgroup = ZALLOC_ARR(pddl_disambiguate_mgroup_t, dis->mgroup_size);
    for (int mi = 0; mi < mgroup.mgroup_size; ++mi){
                     ...
                     pddl_iset_t *exactly_one)
{
    ...
    disamb_set = ZALLOC_ARR(pddl_iset_t, dis->mgroup_size);
