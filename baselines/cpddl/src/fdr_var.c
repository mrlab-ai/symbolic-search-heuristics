...
static void varsMGroupsInit(vars_mgroups_t *vmgs, const pddl_mgroups_t *mgs)
{
    ...
    vmgs->mgroup = ZALLOC_ARR(vars_mgroup_t, vmgs->mgroup_size);
                                         ...
                                         pddl_iset_t *binfs)
{
    ...
    int *mg_facts = ZALLOC_ARR(int, strips->fact.fact_size);
...
static void varsInit(vars_t *vars, const pddl_mgroups_t *mgroups)
{
    ZEROIZE(vars);
    vars->var_alloc = 8;
    vars->var = ZALLOC_ARR(var_t, vars->var_alloc);
                               ...
                               pddl_iset_t *essential)
{
    ...
    fact_mgroups = ZALLOC_ARR(int, strips->fact.fact_size);
                                         ...
                                         const pddl_fdr_vars_config_t *cfg)
{
    ...
    int *covered = ZALLOC_ARR(int, strips->fact.fact_size);
    int fact_id;
                       ...
                       const pddl_strips_t *strips)
{
    ...
    fdr_vars->strips_id_to_val = ZALLOC_ARR(pddl_iset_t,
                                            strips->fact.fact_size);
    fdr_vars->var_size = vars->var_size;
    fdr_vars->var = ZALLOC_ARR(pddl_fdr_var_t, vars->var_size);
    ...
    fdr_vars->global_id_to_val = ZALLOC_ARR(pddl_fdr_val_t *,
    ...
    for (int var_id = 0; var_id < vars->var_size; ++var_id){
        ...
        var->val = ZALLOC_ARR(pddl_fdr_val_t, var->val_size);
...
static void pddlFDRVarCopy(pddl_fdr_var_t *dst, const pddl_fdr_var_t *src)
{
    ...
    dst->val = ZALLOC_ARR(pddl_fdr_val_t, dst->val_size);
...
void pddlFDRVarsInitCopy(pddl_fdr_vars_t *dst, const pddl_fdr_vars_t *src)
{
    ...
    dst->var = ZALLOC_ARR(pddl_fdr_var_t, dst->var_size);
    ...
    if (dst->strips_id_size > 0){
        dst->strips_id_to_val = ZALLOC_ARR(pddl_iset_t, dst->strips_id_size);
                         ...
                         pddl_fdr_vars_remap_t *remap)
{
    ...
    for (int v = 0; v < remap->var_size; ++v){
        remap->remap[v] = ZALLOC_ARR(const pddl_fdr_val_t *,
                                     vars->var[v].val_size);
    }
...
pddl_fdr_var_t *pddlFDRVarsAdd(pddl_fdr_vars_t *vars, int val_size)
{
    ...
    var->val = ZALLOC_ARR(pddl_fdr_val_t, val_size);
    var->val_none_of_those = -1;
