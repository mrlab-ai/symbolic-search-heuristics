% VARIANT: short safety-theory version of the flagship manuscript paper.tex.
% The flagship (paper.tex) folds in the cost-model / pruning material and is
% the intended submission; this file is kept as a fallback short split.
% =========================================================================
...
formal performance guarantee for the operator-potential approach. The
guarantee is one-sided by design: it caps the harm a heuristic can do,
not the benefit.
...
merge-and-shrink family improves broadly on blind forward search.
...
guarantees across zero-cost strata is open. Repeated runs of bidirectional SymK (a baseline below) differ by
one or two solved tasks because its direction selection is timing-based;
we compare runs from a single sweep. We limit time to 30\,minutes and memory to
...
converts its guarantee into a broad coverage improvement over blind
forward search (better in 21 of 46 domains, worse in 5), whose aggregate
margin however rests on few domains. The alignment study adds the cautionary half of the message:
