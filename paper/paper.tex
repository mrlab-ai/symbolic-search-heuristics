...
\bddastar\ pays for its expansion order: partitioning a layer into up to
$\values$ buckets multiplies the number of image operations per layer by
$\values$, a cost that Definition~\ref{def-effort} does not capture
because it counts BDD nodes rather than image calls. This motivates a
...
dead ends from the start. It thereby turns the width bound into a pure
safety guarantee: never more than a factor $Wn$ worse than blind search,
never an extra image operation, and any pruning is a strict saving.
