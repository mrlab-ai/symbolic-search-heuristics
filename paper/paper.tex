...
the image operations performed before termination and $B$ is the argument
of each. Definition~\ref{def-effort} is the case $\alpha = 0$ restricted
to the buckets counted there.
\end{definition}
...
direction schedule, Lemma~\ref{lem-slice} applies within each direction,
so the cumulative expanded size is at most a factor
$\max(\width_\pi(h), \width_\pi(\bar h)) \cdot n$ times that of blind
bidirectional search under the same schedule, again with no additional
image operations. A single linear merge-and-shrink abstraction
...
pruning on up to $20$ tasks per family). Pruning is empirically free---
but in forward search it also gains little, because the anytime upper
bound only becomes available late. Whether the $f$-ordered expansion of
...
pruning-only), while potentials, with a median of $53$ values and $65$
where losses occur, are hurt by it ($622$ vs.\ $683$). The choice between
guidance and safety is thus governed by the number of heuristic values,
exactly as the refined cost model predicts.
...
on ten percent of the tasks). With a $60$-second construction budget and
...
($1041$ vs.\ $1051$) but remains ten tasks behind: three wins from
dead-end pruning do not offset the residual setup and slicing overhead
on tasks that blind search solves close to the time limit. We consider
this an instructive negative result: bidirectional blind search is
robust not because heuristic information fragments its BDDs---our bounds
and measurements rule that out---but because at this level of
performance even provably safe pruning must first pay for computing its
heuristic, and on this suite that constant cost is not recouped.
