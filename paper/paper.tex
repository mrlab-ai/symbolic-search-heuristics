...
requires exponential width, and that the quadratic width factor of the
bound is realized by an explicit family, so the guarantee is tight in
the width. Integer potential heuristics, prefix pattern
...
model that counts image operations then predicts \emph{which} of two
uses of a heuristic pays off: $f$-ordered expansion multiplies image
calls by the number of heuristic values, while a novel
\emph{pruning-only} search keeps blind layer structure, discards
provably useless slices, and enjoys a bound a factor $W$ tighter.
Experiments in SymK confirm both the theory and the model: fragmentation
stays negligible; merge-and-shrink, whose value count is small, improves broadly on
blind forward search with ordering (better in 21 of 46 domains), while
potentials, with many values, only match it via pruning; and in bidirectional search---outside our
guarantee---even provably safe pruning cannot reliably recoup the cost
of computing the heuristic, while an incumbent bound from a satisficing
planner, the cheapest form of guidance, improves even blind search.
\end{abstract}
...
realizes an $\Omega(W^2)$ overhead, so the width factor of our bound is
tight. Fourth, we identify natural width-bounded families:
...
operator-potential approach. Fifth, we refine the cost model of symbolic
search to count image operations, which the node-based effort measure
cannot see, and derive from it a novel \emph{pruning-only} search that
...
fragmentation predicted harmless stays harmless, the refined cost model
correctly predicts that merge-and-shrink profits from $f$-ordering
(improving on blind forward search in 21 of 46 domains) while potential
heuristics do not, and
a candid study of bidirectional search delimits where the guarantee
regime ends and identifies incumbent bounds as the cheapest useful form
of guidance.
...
frontier interacts with forward buckets, is achieved for the
pruning-only variant in Section~\ref{sec-prune} and remains open for
bidirectional \bddastar\ itself.

\subsection{The Factor $W^2$ Is Necessary}

The factor $n$ in our bounds is unavoidable
(Example~\ref{ex-parity}); we now show that the factor $W^{2}$ of
Theorem~\ref{thm-effort} is necessary as well, so that only the factor
$n$ between the two can be loose.
...
admissible heuristics $h$ with $\width_\pi(h) = O(W)$ such that
$\effort(\task_{k,W}, h) = \Omega(W^{2}) \cdot
\effort(\task_{k,W}, \hblind)$.
\end{theorem}

\begin{proof}
The task has binary variables $x_0, \dots, x_k$ (a chain),
$v_1, \dots, v_k$ (choices) and $t_1, \dots, t_W$ (a tail); initially
only $x_0$ holds. All operators cost $1$: for $i \in [k]$, operator
$o_i^1$ has precondition $x_{i-1} \wedge \neg x_i$ and effect
$x_i \wedge v_i$, and $o_i^0$ the same precondition and effect $x_i$;
for $j \in [W]$, operator $u_j$ has precondition
$x_k \wedge t_{j-1} \wedge \neg t_j$ (with $t_0 := x_k$) and effect
$t_j$; the goal is $t_1 \wedge \dots \wedge t_W$, so
$\copt = k + W$. The order $\pi$ lists the
$x$-block, then $v_1, \dots, v_k$, then the $t$-block.

Let $\rho(s) = \bigl(\sum_{j=1}^{k} s(v_j)\bigr) \bmod W$ and define,
over all Boolean assignments,
$h(s) = \min\bigl(\rho(s),\; W - |\{ j \mid s(t_j) = 1 \}|\bigr)$.
Both terms change by at most $1$ per operator application, and on
every assignment satisfying the goal the tail term is $0$, so $h$
vanishes there and $h$ is consistent; $h \leq W - \#t \leq
\hstar$ gives admissibility. Since $h$ ignores the $x$-block, its
cofactors are determined by the partial residue in the $v$-block (at
most $W$ values) and by the pair of final residue and tail count in the
$t$-block, so $\width_\pi(h) = O(W)$.

\emph{Blind effort.} The layer $L_i$ for $i \leq k$ fixes the chain
prefix, sets $v_j$ for $j > i$ and all $t_j$ to false, and leaves
$v_1, \dots, v_i$ unconstrained; free variables contribute no nodes, so
$\bddsize{L_i} = \Theta(k + W)$, and the same holds for the $W$ tail
layers. Hence $\effort(\task_{k,W}, \hblind) = \Theta(k(k+W)) =
\Theta(k^2)$ for $W \leq k$.

\emph{Heuristic effort.} At layer $i \leq k$ all tail variables are
false, so $h(s) = \rho(s) = \bigl(\sum_{j \leq i} s(v_j)\bigr) \bmod W$,
and \bddastar\ splits $L_i$ into the $W$ residue buckets
$L_i \cap \{\rho = c\}$, all nonempty for $i \geq W - 1$ and all
selected, since their keys satisfy $g + v \leq i + W - 1 < \copt$
(Lemma~\ref{lem-identity}). Fix $i \geq 4W$ and a bucket $c$, and
consider prefixes that agree on the chain and differ on
$v_1, \dots, v_j$ with $W \leq j \leq i - W$. Prefixes with distinct
partial residues $\rho_1 \neq \rho_2$ have distinct cofactors: the
suffix that sets exactly $a = (c - \rho_1) \bmod W \leq W - 1 \leq i - j$
of the remaining $v$-variables completes the first prefix to the bucket
but not the second. Moreover, every such cofactor essentially depends on
$v_{j+1}$: its two restrictions $[\rho + \Sigma \equiv c]$ and
$[\rho + 1 + \Sigma \equiv c]$ over the remaining variables are distinct
by the same argument. By canonicity, each of the $\min(j+1, W)$ residue
cofactors at level $j$ is therefore represented by its own inner node
labeled $v_{j+1}$ in the \emph{reduced} OBDD, so
$\bddsize{L_i \cap \{\rho = c\}} \geq (i - 2W) \, W$ from the middle
levels $W \leq j \leq i - W$ alone; this matches the classical
$\Theta(iW)$ size of modular counting functions \citep{wegener-2000}.
Summing over the $W$ buckets and the layers $i \in [4W, k]$,
\[
\effort(\task_{k,W}, h)
\;\geq\; \sum_{i=4W}^{k} W \cdot \Omega(iW)
\;=\; \Omega(k^{2} W^{2}),
\]
which exceeds blind effort by a factor $\Omega(W^{2})$.
\end{proof}

Theorem~\ref{thm-tight} should be read together with the small measured
fragmentation ratios of Section~\ref{sec-experiments}: the quadratic
overhead is realizable, but it requires heuristics whose level sets
entangle the frontier in a way the natural families rarely do.
...
separately (Lemma~\ref{lem-identity}), computing one image each; layer
$g$ has $\values_g \leq |\img(h)| = \values$ nonempty buckets.
\end{proof}

Theorem~\ref{thm-effort} bounds the node term of $\effort_\alpha$ for
every $\alpha$, but no width bound can cap the $\alpha$-term of \bddastar:
the factor $\values_g$ is inherent to expanding buckets separately. Nor
does laziness help: Lazy \bddastar\ \citep{torralba-phd2015} and
SetA\ensuremath{^{*}}/\ghseta\ \citep{jensen-et-al-aij2008} change
\emph{when} the partition is computed, not \emph{which} buckets are
expanded---by the bucket identity (Lemma~\ref{lem-identity}) every
nonempty $(g,v)$ bucket still incurs its own image operation, so
Proposition~\ref{prop-calls} applies to these variants verbatim. The
factor is both constructible and measurable: on the family of
Theorem~\ref{thm-tight}, \bddastar\ performs $\Theta(W)$ image calls per
layer by construction, and empirically the measured calls-per-layer
factor has median $3.0$--$3.2$ on the tasks each configuration solves
but $7.0$--$10.2$ on the tasks it fails while blind search
succeeds---the losses concentrate exactly where the multiplication
bites. This
...
relaxation to be the practical variant. In light of
Section~\ref{sec-cost}, the most consequential synthesis objective is
yet another one: minimizing the number of distinct heuristic
\emph{values} (or operator deltas) attacks the image-call factor at its
source; we return to this in the conclusions. For prefix PDBs, synthesis
...
configurations attempt. The positive-cost restriction is inherited from the
theory: the layer structure underlying the bucket identity
(Lemma~\ref{lem-identity}) requires positive costs, and extending the
guarantees across zero-cost strata is open. Two exceptions to this base are flagged where
...
planner Scorpion \citep{seipp-et-al-jair2020}. The evaluation proceeds in six steps:
does measured fragmentation respect the width bound; how do the families
...
which our implementation does not enforce for the merge order. The slack of the bounds is itself informative: on solved
tasks, even the crude factor $\values$ (one bucket per value) overshoots
the measured fragmentation by median factors of $8$--$26$ across the
families, and the width-based factor $U$ by $120$--$879$; almost all of
it is structure sharing between buckets, as the remark following
Theorem~\ref{thm-partition} predicts. The theorems' quantitative value
is thus the dichotomy they establish together with
Theorems~\ref{thm-tight} and~\ref{thm-lb}---bounded width cannot blow up,
unbounded width can---rather than a tight per-task prediction.
...
(Theorem~\ref{thm-lb}), the very blow-up that width-boundedness rules out. Running the width-bounded families on $\task_n$ completes the
contrast: capped potentials and prefix PDBs keep every expanded bucket
linear as well (at most $50$ and $83$ nodes up to $n = 12$), succeeding
exactly where the perfect heuristic provably cannot.
...
discussed below. The result we do claim
...
number of heuristic values, exactly as the refined cost model predicts. The prediction also holds per task and per domain: among
tasks solved by exactly one of a family's two variants, ordering wins at
a median of $21$ heuristic values and loses at a median of $59$; per
domain, ordering is better in $22$ domains and worse in $5$ for
merge-and-shrink, but better in only $9$ and worse in $20$ for
potentials.
...
positively: pruning-only potentials reach $688$ positive-cost tasks
...
into a safety knob (Proposition~\ref{prop-ms}), and the image-call
analysis (Proposition~\ref{prop-calls}) that predicts---correctly, per
Section~\ref{sec-experiments}---that this family profits from
$f$-ordering precisely because its value count is small. The
...
under the factor-$Wn$ safety bound of Corollary~\ref{cor-prune} and with
a construction budget that bounds the total overhead including setup.
...
exponential width, and an explicit family realizes the quadratic width
factor, making the bound tight in $W$. Potential heuristics, prefix PDBs and linear
...
Beyond the width theory, an image-call-aware cost model exposed the
operational price of $f$-ordered expansion, and the derived pruning-only
search carries a bound a factor $W$ tighter with the image-call profile
of blind search. The experiments matched the theory unusually closely:
fragmentation is negligible wherever width is bounded; pruning-only
search is coverage-neutral and effort-neutral, as guaranteed;
merge-and-shrink, whose value count is small, converts ordering into a
coverage improvement over blind forward search; and potentials, with
many values, cannot. In bidirectional search, outside our guarantee,
four successively more careful designs---up to seeding an incumbent from
a satisficing planner---consistently showed that even provably safe
pruning does not reliably recoup the cost of computing the heuristic
against SymK's blind bidirectional search, while the incumbent itself
proved to be the cheapest useful form of guidance: it improves blind
bidirectional search and turns forward pruning-only potentials into a
net gain. We see three open
directions: a formal effort guarantee for bidirectional \bddastar\ that
accounts for the bound-sequence feedback identified in
Section~\ref{sec-prune}; merge and variable orders chosen \emph{jointly}
to balance representational width against abstraction accuracy---our
alignment experiment shows that optimizing width alone sacrifices
accuracy---and heuristics engineered directly for few
distinct values, small cofactor width, high accuracy and cheap
construction---the quantities our analysis identifies as decisive, the
first being the synthesis objective that Section~\ref{sec-synthesis}
motivates but leaves open.
