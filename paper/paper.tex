...
(surpassing blind forward search) while potential heuristics do not, and
...
$t_j$; the goal is $t_W$, so $\copt = k + W$. The order $\pi$ lists the
...
Both terms change by at most $1$ per operator application and $h$
vanishes on goal states, so $h$ is consistent; $h \leq W - \#t \leq
