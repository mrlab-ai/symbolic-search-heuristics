...
stays negligible; merge-and-shrink, whose value count is small, beats
blind forward search with ordering, while potentials, with many values,
only match it via pruning; and in bidirectional search---outside our
...
three successively more careful designs consistently showed that even
provably safe pruning does not reliably recoup the cost of computing the
heuristic against SymK's blind bidirectional search. We see three open
...
Section~\ref{sec-prune}; merge and variable orders chosen jointly so
that the closed-form width bounds apply a priori; and heuristics
engineered directly for small cofactor width and cheap construction,
the two quantities our analysis identifies as decisive.
