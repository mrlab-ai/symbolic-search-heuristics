...
% Experiments section contains TODO placeholders pending implementation.
% =========================================================================
...
\title{Width-Bounded Heuristics for Symbolic Search}
...
% ============================== abstract =================================
\begin{abstract}
Symbolic search with binary decision diagrams (BDDs) is a leading approach
to cost-optimal classical planning, but its dominant instantiation remains
blind: heuristics partition the search frontier by heuristic value, and
this partitioning can increase BDD sizes so much that even the perfect
heuristic deteriorates search performance exponentially. This negative
result leaves open the central question of when heuristics provably help
symbolic search. We answer it with a sufficient condition. We introduce
\emph{width-bounded heuristics}, whose value is computable by scanning the
state variables once in the BDD variable order while distinguishing at
most $W$ cases at any point. We prove that partitioning \emph{any} state
set by the values of a width-$W$ heuristic increases the total BDD size by
at most a factor polynomial in $W$ and the number of variables, and that
symbolic \astar\ with a
consistent width-$W$ heuristic incurs at most a polynomial-factor overhead
over blind symbolic search---in contrast to the exponential deterioration
possible in general, which we show requires exponential width. Potential
heuristics, pattern databases over pattern prefixes and linear
merge-and-shrink abstractions are width-bounded, which explains the
empirical success of operator-potential heuristics. Finally, we show how
to synthesize informative width-bounded potential heuristics, turning the
width bound into a knob that trades heuristic accuracy against a
guaranteed limit on representational overhead.
\end{abstract}
...
as binary decision diagrams \citep[BDDs;][]{bryant-tc1986}. Symbolic
...
heuristics}: a heuristic has width $W$ under a variable order if its
value can be determined by reading the state variables once in that
order while maintaining at most $W$ distinct cases at any point,
...
operator-potential approach. Finally, we describe how to synthesize
informative width-bounded potential heuristics and outline an empirical
evaluation in the SymK planner \citep{speck-et-al-jair2025}.
...
\citep{bryant-tc1986}. We write $\bddsize{S}$ for the number of inner
...
\section{Width-Bounded Heuristics}
...
\label{def-width}
The \emph{width} of a heuristic $h$ under variable order $\pi$ is
$\width_\pi(h) = \max_{0 \leq i \leq n} |\sub{i}{h}|$. We call $h$
\emph{$(\pi,W)$-width-bounded} if $\width_\pi(h) \leq W$.
\end{definition}
...
$\pi$. Three immediate consequences will do much of the work below.
...
\citep{bryant-tc1986}. Applied naively with $g = \chi_{\level{h}{v}}$ it
...
role. \citet{torralba-phd2015} proved that linear M\&S heuristics have
...
uses it. In contrast, no small-width representation is known for
...
% TODO: run experiments; all numbers below are placeholders.
We implemented width instrumentation, width-capped potential heuristics
and prefix-PDB heuristics in the SymK planning system
\citep{speck-et-al-jair2025}, which extends Fast Downward
\citep{helmert-jair2006}, and used the Downward Lab toolkit
\citep{seipp-et-al-zenodo2017} for running experiments.
% TODO: hardware.
Our benchmark set consists of all tasks without conditional effects and
axioms from the optimal sequential tracks of the IPCs 1998--2023,
matching the suite of \citet{fiser-et-al-aij2024}. We limit time to
30\,minutes and memory to 8\,GiB. All benchmarks, code and experiment
data are available online.
% TODO: Zenodo record.

We focus on coverage (number of solved tasks), the expansion size
(Definition~\ref{def-effort}) measured in BDD nodes, peak memory and
the \emph{fragmentation ratio}, i.e., the total size of the buckets of
a layer divided by the size of the unpartitioned layer BDD, which
Theorem~\ref{thm-partition} predicts to be at most polynomial in the
measured width. We compare against symbolic bidirectional blind search
in SymK, forward blind search, the operator-potential configurations of
\citet{fiser-et-al-aij2024} and \symba\ \citep{torralba-et-al-aij2017}.
Our evaluation addresses four questions. (Q1)~Does the measured
fragmentation ratio respect the width bound, and does width predict
fragmentation across domains? (Q2)~How do coverage and expansion size
change with the width budget $M$? (Q3)~Do width-capped potentials
recover the domains in which unconstrained operator potentials lose to
blind search while preserving their gains? (Q4)~How does the resulting
configuration compare per domain to the state of the art in symbolic
planning?
% TODO: Tables 1-4 with results for Q1-Q4; fix winning M value after Q2.
...
\citet{edelkamp-et-al-ecai2012} compiled PDB and M\&S heuristics into
vectors of BDDs for \bddastar, \citet{torralba-phd2015} proved
polynomial-size BDD representations for linear M\&S and analyzed
ADD-to-BDD conversions, and \citet{helmert-et-al-icaps2015} introduced
factored mappings, which our width notion relativizes to the search
order; none of these works bounds the effect of the representation on
the search frontier, which is our contribution. The operator-potential
line \citep{fiser-et-al-aaai2022,fiser-et-al-aij2024} demonstrated
...
heuristics \citep{torralba-et-al-ijcai2016,torralba-et-al-aij2018},
which defer heuristic computation until blind search stalls, variable
ordering analyses \citep{kissmann-hoffmann-jair2014} and EVMDD-based
representations of state-dependent cost structure
\citep{speck-et-al-icaps2018}.

% ============================ conclusions ================================
\section{Conclusions}

We introduced width-bounded heuristics and proved that bounded width is
a sufficient condition for heuristics to be provably safe in symbolic
search: partitioning any state set by heuristic values costs at most a
polynomial factor in the width, and \bddastar\ with a consistent
width-$W$ heuristic incurs at most an $O(W^2 n)$ overhead over blind
symbolic forward search, whereas the known exponential deterioration
requires exponential width. Potential heuristics, prefix PDBs and
linear merge-and-shrink instantiate the class with widths controlled by
the potential magnitude, the abstract state count and the shrink limit,
respectively. In future work, we want to extend the guarantee to
bidirectional symbolic search, study whether width-aware variable
ordering can enlarge the class in practice, and investigate
size-monitored search regimes for heuristics outside the class.

% ------------------------------------------------------------------------
\begin{thebibliography}{99}

\bibitem[B{\"a}ckstr{\"o}m and Nebel(1995)]{backstrom-nebel-compint1995}
C.~B{\"a}ckstr{\"o}m and B.~Nebel.
\newblock Complexity results for SAS$^{+}$ planning.
\newblock \emph{Computational Intelligence}, 11(4):625--655, 1995.

\bibitem[Bahar et~al.(1993)]{bahar-et-al-iccad1993}
R.~I. Bahar, E.~A. Frohm, C.~M. Gaona, G.~D. Hachtel, E.~Macii, A.~Pardo
  and F.~Somenzi.
\newblock Algebraic decision diagrams and their applications.
\newblock In \emph{Proc.\ ICCAD 1993}, pages 188--191, 1993.

\bibitem[Bryant(1986)]{bryant-tc1986}
R.~E. Bryant.
\newblock Graph-based algorithms for Boolean function manipulation.
\newblock \emph{IEEE Transactions on Computers}, 35(8):677--691, 1986.

\bibitem[Culberson and Schaeffer(1998)]{culberson-schaeffer-compint1998}
J.~C. Culberson and J.~Schaeffer.
\newblock Pattern databases.
\newblock \emph{Computational Intelligence}, 14(3):318--334, 1998.

\bibitem[Edelkamp(2001)]{edelkamp-ecp2001}
S.~Edelkamp.
\newblock Planning with pattern databases.
\newblock In \emph{Proc.\ ECP 2001}, pages 84--90, 2001.

\bibitem[Edelkamp and Helmert(1999)]{edelkamp-helmert-ecp1999}
S.~Edelkamp and M.~Helmert.
\newblock Exhibiting knowledge in planning problems to minimize state
  encoding length.
\newblock In \emph{Proc.\ ECP 1999}, pages 135--147, 1999.

\bibitem[Edelkamp and Reffel(1998)]{edelkamp-reffel-ki1998}
S.~Edelkamp and F.~Reffel.
\newblock {OBDDs} in heuristic search.
\newblock In \emph{Proc.\ KI 1998}, pages 81--92, 1998.

\bibitem[Edelkamp et~al.(2012)]{edelkamp-et-al-ecai2012}
S.~Edelkamp, P.~Kissmann and {\'A}.~Torralba.
\newblock Symbolic {A}$^{*}$ search with pattern databases and the
  merge-and-shrink abstraction.
\newblock In \emph{Proc.\ ECAI 2012}, pages 306--311, 2012.

\bibitem[Fi{\v s}er et~al.(2022)]{fiser-et-al-aaai2022}
D.~Fi{\v s}er, {\'A}.~Torralba and J.~Hoffmann.
\newblock Operator-potential heuristics for symbolic search.
\newblock In \emph{Proc.\ AAAI 2022}, pages 9750--9757, 2022.

\bibitem[Fi{\v s}er et~al.(2024)]{fiser-et-al-aij2024}
D.~Fi{\v s}er, {\'A}.~Torralba and J.~Hoffmann.
\newblock Boosting optimal symbolic planning: Operator-potential
  heuristics.
\newblock \emph{Artificial Intelligence}, 334:104174, 2024.

\bibitem[Hansen et~al.(2002)]{hansen-et-al-sara2002}
E.~A. Hansen, R.~Zhou and Z.~Feng.
\newblock Symbolic heuristic search using decision diagrams.
\newblock In \emph{Proc.\ SARA 2002}, pages 83--98, 2002.

\bibitem[Hart et~al.(1968)]{hart-et-al-ieeessc1968}
...
\bibitem[Helmert(2006)]{helmert-jair2006}
M.~Helmert.
\newblock The {Fast Downward} planning system.
\newblock \emph{Journal of Artificial Intelligence Research},
  26:191--246, 2006.

\bibitem[Helmert et~al.(2014)]{helmert-et-al-jacm2014}
M.~Helmert, P.~Haslum, J.~Hoffmann and R.~Nissim.
\newblock Merge-and-shrink abstraction: A method for generating lower
  bounds in factored state spaces.
\newblock \emph{Journal of the ACM}, 61(3):16:1--16:63, 2014.

\bibitem[Helmert et~al.(2015)]{helmert-et-al-icaps2015}
M.~Helmert, G.~R{\"o}ger and S.~Sievers.
\newblock On the expressive power of non-linear merge-and-shrink
  representations.
\newblock In \emph{Proc.\ ICAPS 2015}, pages 106--114, 2015.

\bibitem[Jensen et~al.(2008)]{jensen-et-al-aij2008}
R.~M. Jensen, M.~M. Veloso and R.~E. Bryant.
\newblock State-set branching: Leveraging {BDDs} for heuristic search.
\newblock \emph{Artificial Intelligence}, 172(2--3):103--139, 2008.

\bibitem[Kissmann and Edelkamp(2011)]{kissmann-edelkamp-aaai2011}
P.~Kissmann and S.~Edelkamp.
\newblock Improving cost-optimal domain-independent symbolic planning.
\newblock In \emph{Proc.\ AAAI 2011}, pages 992--997, 2011.

\bibitem[Kissmann(2012)]{kissmann-phd2012}
P.~Kissmann.
\newblock \emph{Symbolic Search in Planning and General Game Playing}.
\newblock PhD thesis, University of Bremen, 2012.

\bibitem[Kissmann and Hoffmann(2014)]{kissmann-hoffmann-jair2014}
P.~Kissmann and J.~Hoffmann.
\newblock {BDD} ordering heuristics for classical planning.
\newblock \emph{Journal of Artificial Intelligence Research},
  51:779--804, 2014.

\bibitem[McMillan(1993)]{mcmillan-1993}
K.~L. McMillan.
\newblock \emph{Symbolic Model Checking}.
\newblock Kluwer Academic Publishers, 1993.

\bibitem[Pearl(1984)]{pearl-1984}
J.~Pearl.
\newblock \emph{Heuristics: Intelligent Search Strategies for Computer
  Problem Solving}.
\newblock Addison-Wesley, 1984.

\bibitem[Pommerening et~al.(2015)]{pommerening-et-al-aaai2015}
F.~Pommerening, M.~Helmert, G.~R{\"o}ger and J.~Seipp.
\newblock From non-negative to general operator cost partitioning.
\newblock In \emph{Proc.\ AAAI 2015}, pages 3335--3341, 2015.

\bibitem[Seipp et~al.(2017)]{seipp-et-al-zenodo2017}
J.~Seipp, F.~Pommerening, S.~Sievers and M.~Helmert.
\newblock {Downward Lab}.
\newblock Zenodo, 2017.

\bibitem[Speck and Helmert(2025)]{speck-helmert-ecai2025}
D.~Speck and M.~Helmert.
\newblock On performance guarantees for symbolic search in classical
  planning.
\newblock In \emph{Proc.\ ECAI 2025}, pages 4628--4636, 2025.

\bibitem[Speck et~al.(2018)]{speck-et-al-icaps2018}
D.~Speck, F.~Gei{\ss}er and R.~Mattm{\"u}ller.
\newblock Symbolic planning with edge-valued multi-valued decision
  diagrams.
\newblock In \emph{Proc.\ ICAPS 2018}, pages 250--258, 2018.

\bibitem[Speck et~al.(2020)]{speck-et-al-icaps2020}
D.~Speck, F.~Gei{\ss}er and R.~Mattm{\"u}ller.
\newblock When perfect is not good enough: On the search behaviour of
  symbolic heuristic search.
\newblock In \emph{Proc.\ ICAPS 2020}, pages 263--271, 2020.

\bibitem[Speck et~al.(2025)]{speck-et-al-jair2025}
D.~Speck, J.~Seipp and {\'A}.~Torralba.
\newblock Symbolic search for cost-optimal planning with expressive
  model extensions.
\newblock \emph{Journal of Artificial Intelligence Research},
  82:1349--1405, 2025.

\bibitem[Torralba(2015)]{torralba-phd2015}
{\'A}.~Torralba.
\newblock \emph{Symbolic Search and Abstraction Heuristics for
  Cost-Optimal Planning}.
\newblock PhD thesis, Universidad Carlos III de Madrid, 2015.

\bibitem[Torralba et~al.(2016)]{torralba-et-al-ijcai2016}
{\'A}.~Torralba, C.~Linares~L{\'o}pez and D.~Borrajo.
\newblock Abstraction heuristics for symbolic bidirectional search.
\newblock In \emph{Proc.\ IJCAI 2016}, pages 3272--3278, 2016.

\bibitem[Torralba et~al.(2017)]{torralba-et-al-aij2017}
{\'A}.~Torralba, V.~Alc{\'a}zar, P.~Kissmann and S.~Edelkamp.
\newblock Efficient symbolic search for cost-optimal planning.
\newblock \emph{Artificial Intelligence}, 242:52--79, 2017.

\bibitem[Torralba et~al.(2018)]{torralba-et-al-aij2018}
{\'A}.~Torralba, C.~Linares~L{\'o}pez and D.~Borrajo.
\newblock Symbolic perimeter abstraction heuristics for cost-optimal
  planning.
\newblock \emph{Artificial Intelligence}, 259:1--31, 2018.

\bibitem[Wegener(2000)]{wegener-2000}
I.~Wegener.
\newblock \emph{Branching Programs and Binary Decision Diagrams}.
\newblock SIAM, 2000.
